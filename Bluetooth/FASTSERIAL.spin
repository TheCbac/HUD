...
PUB tx(txbyte)
  repeat while txhold           ' wait until txhold is released
  txhold := txbyte|$8000_0000   ' set it and flag it
...
entry                   mov     t1,par                'get structure address
        ...
        if_nz           or      dira,tepin              ' activate te
                        ...
                        add     t1,#4
                        mov     txholdP,t1                   ' set pointer to txhold
                        wrlong  zero,txholdP
...
rxstore
                        ...
                        add     t2,rxbuff               ' point to rxbuffer in hub
                        wrbyte  rxdata,t2
                        sub     t2,rxbuff
                        add     t2,#1
                        and     t2,#rxsz-1
...
transmit                jmpret  txcode,rxcode         'run a chunk of receive code, then return
                        rdlong  txdata,txholdP               ' read txhold register for character or stringptr
                        test    txdata,strflg  wz      ' strings will have high order bit set
        if_nz           jmp     #transmitstr
                        test    txdata,chflg wz
        if_nz           wrlong  zero,txholdP
        if_nz           jmp     #transmitchar                    
                        jmp     #transmit

transmitstr
                        wrlong  zero,txholdP              ' clear txhold
                        andn    t3,strflg
...
t2                      res     1
t3                      res     1
txholdP                 res     1               'pointer to txhold
rxheadP                 res     1
rxgapP                  res     1
config                  res     1
bitticks                res     1
